...
class QuickUnion:
    r"""
    This class provides methods for the quick union (weighted) algorithm.
    ...
    def __init__(self, length: int):
        ...
        self.__list = []  # Initialize empty lists.
        self.__size = []
        for i in range(0, length):
            self.__list.append(i)  # Set the default root of each index to itself.
            self.__size.append(1)  # Set the default size of each index to 1.
    ...
    def getRoot(self, x: int) -> int:
        ...
        while self.__list[x] != x:  # Check if the ID of x is equal to itself.
            x = self.__list[x]  # Check the parent in the next iteration if it is the root.
    ...
    def union(self, p: int, q: int) -> None:
        ...
        if self.__size[p] > self.__size[q]:
            # If the size of p is larger than the size of q,
            # Move q to p.
            self.__list[self.getRoot(q)] = p
            self.__size[p] += self.__size[q]  # Add the size of q to size of p.

        elif self.__size[p] < self.__size[q]:
            # If the size of p is less than the size of q,
            # Move p to q.
            self.__list[self.getRoot(p)] = q
            self.__size[q] += self.__size[p]  # Add the size of p to size of q.

        else:
            # If both are the same, either way is fine.
            self.__list[self.getRoot(p)] = q
            self.__size[q] += self.__size[p]  # Add the size of p to size of q.
...
def main():
    ...
    while True:
        ...
        for i, group in enumerate(groups):
            ...
            print(f"\tGroup #{group}: {', '.join(str(i) for i in groups[group])} (size: {QUDemo.getSize(group)})")
        ...
        if option == 99:
            ...
            print(f"Index {p}: parent: {QUDemo.getParent(p)}; root: {QUDemo.getRoot(p)}; tree size: {QUDemo.getSize(p)})")
            print(f"Index {q}: parent: {QUDemo.getParent(q)}; root: {QUDemo.getRoot(q)}; tree size: {QUDemo.getSize(q)})")
            print(f"Time spent: {total_time}")

        elif option == 2:
            p = int(input("Enter p: "))
            q = int(input("Where to move p? Enter q: "))
            print()
            print("Before Union:")
            ...
            for index, group in enumerate(QUDemo._getList()):
                ...
                print(f"Index {index}: parent: {QUDemo.getParent(index)}; root: {QUDemo.getRoot(index)}; tree size: {QUDemo.getSize(index)})")
