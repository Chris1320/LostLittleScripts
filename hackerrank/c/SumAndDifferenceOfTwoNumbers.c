 ...
 *
 * The `printf()` function prints the given statement to the
 * console. The syntax is `printf("format string",argument_list);`.
 * In the function, if we are using an integer, character, string
 * or float as argument, then in the format string we have to write
 * %d (integer), %c (character), %s (string), %f (float) respectively.
 *
 * The `scanf()` function reads the input data from the console.
 * The syntax is `scanf("format string",argument_list);`. For ex:
 * The `scanf("%d",&number)` statement reads integer number from the
 * console and stores the given value in variable number.
 *
 * To input two integers separated by a space on a single line, the
 * command is `scanf("%d %d", &n, &m)`, where n and m are the two integers.
 *
 * Task
 ...
 *
 * Explanation
 * When we sum the integers 10 and 4, we get the integer 14. When we subtract the second
 * number 4 from the first number 10, we get 6 as their difference.
 * When we sum the floating-point numbers 4.0 and 2.0, we get 6.0. When we subtract the
 * second number 2.0 from the first number 4.0, we get 2.0 as their difference.
 */
