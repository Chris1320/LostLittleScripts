 ...
 * `printf("%c", ch)` writes a character specified by the argument char to stdout
 *
 * ```c
 * char ch;
 * scanf("%c", &ch);
 ...
 *
 * You can take a string as input in C using `scanf(“%s”, s)`. But, it accepts string
 * only until it finds the first space.
 *
 * In order to take a line as input, you can use `scanf("%[^\n]%*c", s);` where s is
 * defined as `char s[MAX_LEN]` where `MAX_LEN` is the maximum size of s. Here, `[]` is
 * the scanset character. `^\n` stands for taking input until a newline isn't encountered.
 * Then, with this `%*c`, it reads the newline character and here, the used `*` indicates
