...
can use Python to check some individual numbers. Maybe you'll even find the one which
would disprove the hypothesis.

Write a program which reads one natural number and executes the above steps as long as
c0 remains different from 1. Moreover, we'll add another task - we want you to count
the steps needed to achieve the goal. Your code should output all the intermediate
values of c0, too.
...
def main(arg: int | None = None) -> None:
    steps: int = 0
    c0: int = arg if arg is not None else int(input("Enter a number >> "))
