...
void loop()
{
    ...
    brightness_value += fading_increment; // increment or decrement brightness depending on fading value
    delay(DELAY_LEN);                        // slow down the loop to allow the user to view the process
    if (brightness_value == 0 || brightness_value == 255)
        fading_increment = -fading_increment; // invert the value of fading if we have reached max value
}
