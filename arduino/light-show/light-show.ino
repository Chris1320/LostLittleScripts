...
// Turn off all LEDs
void offAllNumbers() {
  for (int i = LED_PIN_START; i <= LED_PIN_END; i++) digitalWrite(i, LOW);
}

void strobeLights() {
  // Turn on LEDs one-by-one.
  for (int active_led = LED_PIN_START; active_led <= LED_PIN_END;
       active_led++) {
...
void blinkLights() {
  ...
  while (current_delay > 0) {
    ...
    led_is_on = !led_is_on;
    current_delay -=
        decrement;  // subtract the time taken from the current delay
  }
}
